...
export default async function ProjectDetailsServerSideWidget({ params }) {
    const { id } = params || {};
    if (!id) {
        return <div className="p-4 text-red-300">Project id is missing.</div>;
    }

    // list() у нас иногда возвращает {items}, иногда массив — нормализуем
    const [nsRes, svcRes] = await Promise.all([
        NamespaceRepo.list(id).catch(() => ({ items: [] })),
        ServiceRepo.list(id).catch(() => ({ items: [] })),
    ]);
    const namespaces = Array.isArray(nsRes) ? nsRes : (nsRes?.items || []);
    const services   = Array.isArray(svcRes) ? svcRes : (svcRes?.items || []);
...
async function countBindings(projectId, services) {
    ...
    for (const s of services) {
        const nsIds = await BindingRepo.listByService(projectId, s.id).catch(() => []);
