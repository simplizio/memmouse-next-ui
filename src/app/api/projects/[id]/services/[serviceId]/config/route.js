...
//
// export async function GET(req, { params }) {
//     const { id, serviceId } = await params; // projectId, serviceId
//     const auth = req.headers.get("authorization") || "";
//     const token = auth.startsWith("Bearer ") ? auth.slice(7) : null;
//
//     const svc = await ServiceRepo.get(id, serviceId);
...
//     if (!ok) return NextResponse.json({ error:"unauthorized" }, { status:401 });
//
//     // собрать биндинги сервиса по всем неймспейсам проекта
//     const nsIds = await redis.smembers(`mm:idx:namespaces:${id}`);
//     const bindings = [];
//     for (const nsId of nsIds) {
//         const b = await BindingRepo.get(id, nsId, serviceId);
//         if (b) bindings.push({ nsId, ...b });
//     }
//
//     const cfg = {
//         version: 1,
...
//         service: { id: svc.id, name: svc.name, scopes: svc.scopes || [] },
//         endpoints: {
//             data: process.env.REDIS_URL,         // демо: тот же редис
//             control: process.env.APP_URL || "http://localhost:3000"
//         },
