...
//     const body = await req.json().catch(() => ({}));
//     const name = String(body.name || "").trim();
//     const scopes = Array.isArray(body.scopes) ? body.scopes.map(s => String(s)) : [];
//
//     if (!name) {
...
//     const service = {
//         projectId,
//         id: (body.id && String(body.id).trim()) || ulid(),  // ← генерим, если не пришёл
//         name,
//         scopes,
//         tokens: [{ id: "t1", value: makeToken(), createdAt: now, status: "active" }],
//         createdAt: now,
...
//     };
//
//     // upsert безопаснее, но если у тебя только create — оставь create
//     const saved = await ServiceRepo.upsert ? ServiceRepo.upsert(service) : ServiceRepo.create(service);
//
//     // ← возвращаем в обёртке, как ждёт ServiceWizard
